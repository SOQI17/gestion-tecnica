...
Torre Titanium,REP-CSV-053,CCTV Bosch 48 Cams,2026-03-15,Marzo,Semana 11,SI,Limpieza óptica de domos exteriores y calibración de feeds.,Andrés Castro`;
  ...
  const engineerStats = React.useMemo(() => {
    ...
    filteredDashOrders.forEach(wo => {
      ...
      const isCompleted = effStatus === 'Realizado' || effStatus === 'Reportado' || effStatus === 'Conciliado';
      ...
      if (statsMap[wo.engineerId]) {
        ...
        if (wo.type === 'Preventivo') {
          statsMap[wo.engineerId].preventiveCount++;
          if (isCompleted) statsMap[wo.engineerId].preventiveCompletedCount++;
          ...
          statsMap[wo.engineerId].otherCount++;
          if (isCompleted) statsMap[wo.engineerId].otherCompletedCount++;
        }
      }
      const supportIds = wo.supportEngineerIds && wo.supportEngineerIds.length > 0
        ? wo.supportEngineerIds
      ...
      supportIds.forEach(id => {
        if (statsMap[id]) {
          ...
          if (wo.type === 'Preventivo') {
            statsMap[id].preventiveCount++;
            if (isCompleted) statsMap[id].preventiveCompletedCount++;
          } else if (wo.type === 'Correctivo') {
            statsMap[id].correctiveCount++;
            if (isCompleted) statsMap[id].correctiveCompletedCount++;
          } else if (isInstallation) {
            // User requirement: Only primary engineers receive installation closure rate tracking; support engineers get dash (-)
          } else {
            statsMap[id].otherCount++;
            if (isCompleted) statsMap[id].otherCompletedCount++;
          }
        }
      });
    });
          ...
          <table>
            ...
            <tbody>
              ${engineerStats.map(st => {
                ...
                const parts: string[] = [];
                if (st.preventiveCount > 0) parts.push(`${st.preventiveCount} Prev`);
                if (st.correctiveCount > 0) parts.push(`${st.correctiveCount} Corr`);
                if (st.installationWOCount > 0) parts.push(`${st.installationWOCount} Inst`);
                if (st.otherCount > 0) parts.push(`${st.otherCount} Otros`);
                const typeSummary = parts.length > 0 ? parts.join(' / ') : '0 Tareas';
  ...
  const renderAgendamientoTab = () => {
    return (
      <AgendamientoTab
        ...
        setNewWOClient={setNewWOClient}
        setNewWOClientSearch={setNewWOClientSearch}
        setNewWOEquipment={setNewWOEquipment}
        setNewWONotes={setNewWONotes}
        setNewWOSupportEngineers={setNewWOSupportEngineers}
        setNewWOSupportEngineer={setNewWOSupportEngineer}
        setNewWOType={setNewWOType}
        setNewWOTimeStart={setNewWOTimeStart}
        setNewWOTimeEnd={setNewWOTimeEnd}
        setNewWODurationDays={setNewWODurationDays}
        setNewWODate={setNewWODate}
        setWoEngDropdownOpen={setWoEngDropdownOpen}
        setWoEngSearchQuery={setWoEngSearchQuery}
        setIsCreatingWO={setIsCreatingWO}
        currentDateStr={currentDateStr}
        handlePrintCalendar={handlePrintCalendar}
        handleExportCalendarExcel={handleExportCalendarExcel}
        handleSmartReorganize={handleSmartReorganize}
        filterOnlyConflicting={filterOnlyConflicting}
        setFilterOnlyConflicting={setFilterOnlyConflicting}
        conflictingWOIds={conflictingWOIds}
        setIsReportMonthModalOpen={setIsReportMonthModalOpen}
        currentMonthWOs={currentMonthWOs}
        setIsResetModalOpen={setIsResetModalOpen}
        handleLoadSamplePlanificacion={handleLoadSamplePlanificacion}
        handleLoadSampleReportes={handleLoadSampleReportes}
        ...
        handleImportYearChange={handleImportYearChange}
        importMonth={importMonth}
        handleImportMonthChange={handleImportMonthChange}
        detectedFormatType={detectedFormatType}
        importFeedback={importFeedback}
        parsedOrders={parsedOrders}
        handleCommitImport={handleCommitImport}
        clients={clients}
        parsedClients={parsedClients}
        parsedEngineers={parsedEngineers}
        ...
        highlightedEngineerId={highlightedEngineerId}
        setHighlightedEngineerId={setHighlightedEngineerId}
        monthEngineers={monthEngineers}
        getEngineerEmoji={getEngineerEmoji}
        searchQuery={searchQuery}
        setSearchQuery={setSearchQuery}
        isReorganizePreviewMode={isReorganizePreviewMode}
        reassignedWOIds={reassignedWOIds}
        handleApplyReorganization={handleApplyReorganization}
        handleCancelReorganizationPreview={handleCancelReorganizationPreview}
        calendarDays={calendarDays}
        calendarMonthName={calendarMonthName}
        workOrders={workOrders}
        getEngineerColorClasses={getEngineerColorClasses}
        matchesSearch={matchesSearch}
        setInfoWO={setInfoWO}
        getEngineerFullNameNoTitle={getEngineerFullNameNoTitle}
        auditorStyle={auditorStyle}
        setAuditorStyle={setAuditorStyle}
        auditorMonth={auditorMonth}
        setAuditorMonth={setAuditorMonth}
        auditorYear={auditorYear}
        setAuditorYear={setAuditorYear}
        monthsList={monthsList}
        selectedEngTab={selectedEngTab}
        setSelectedEngTab={setSelectedEngTab}
        reports={reports}
        onValidateReport={onValidateReport}
        selectedAuditWOId={selectedAuditWOId}
        setSelectedAuditWOId={setSelectedAuditWOId}
        isRechazando={isRechazando}
        setIsRechazando={setIsRechazando}
        validationNotes={validationNotes}
        setValidationNotes={setValidationNotes}
        setSelectedRETE04WOId={setSelectedRETE04WOId}
        setIsViewingRETE04={setIsViewingRETE04}
        ...
        showEngFilterModal={showEngFilterModal}
        setShowEngFilterModal={setShowEngFilterModal}
        excludedEngIds={excludedEngIds}
        setExcludedEngIds={setExcludedEngIds}
        filteredDashOrders={filteredDashOrders}
        dashYear={dashYear}
        ...
        dashSemester={dashSemester}
        setDashSemester={setDashSemester}
        handleExportDashboardCSV={handleExportDashboardCSV}
        handlePrintMainDashboard={handlePrintMainDashboard}
        expandedMainKPICard={expandedMainKPICard}
        setExpandedMainKPICard={setExpandedMainKPICard}
        dashboardKPIs={dashboardKPIs}
        getWOEffectiveStatus={getWOEffectiveStatus}
        getWOClientDisplayName={getWOClientDisplayName}
        isInstallationWO={isInstallationWO}
        engineerStats={engineerStats}
        setSelectedEngForMetrics={setSelectedEngForMetrics}
        setIsEngMetricsModalOpen={setIsEngMetricsModalOpen}
      />
    );
  };
